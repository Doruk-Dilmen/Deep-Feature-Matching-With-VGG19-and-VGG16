...
class DeepFeatureMatcher(torch.nn.Module):
    ...
    def __init__(self, model: str = 'VGG16BN', device = None, bidirectional=True, enable_two_stage = True, ratio_th = [0.9, 0.9, 0.9, 0.9, 0.95, 1.0]):
        ...
        if model == 'VGG16':
            print('loading VGG16...')
            self.model = Vgg16(batch_normalization = False).to(self.device)
            self.padding_n = 16
            print('model is loaded.')
        elif model == 'VGG16_BN':
            print('loading VGG16_BN...')
            self.model = Vgg16(batch_normalization = True).to(self.device)
            self.padding_n = 16
            print('model is loaded.')
        else:
...
class Vgg16(torch.nn.Module):
    ...
    def __init__(self, batch_normalization = True, required_layers = None):
        ...
        if required_layers == None and batch_normalization:
            ...
            required_layers = [2, 7, 10, 16, 27, 29]
            # required_layers = [3, 7, 12, 20, 28, 30]
            # required_layers = [3, 8, 15, 22, 26, 30]
            # required_layers = [4, 9, 16, 23, 28, 30]
        
        super(Vgg16, self).__init__()
        features = list(models.vgg16(pretrained=True).features)[:31]
        self.features = torch.nn.ModuleList(features).eval()
        
	#if batch_normalization:
        #    features = list(models.vgg16_bn(pretrained = True).features)[:47] # get vgg features
        #else:
        #    features = list(models.vgg16(pretrained = True).features)[:33] # get vgg features
